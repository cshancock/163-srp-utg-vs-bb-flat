...
But I think there's a downside to using it as a clustering variable. First, it collapses some of the betting nuances down to a single number. If a flop is bet for the large bet amount a large percentage of the time, we want to know that, and we probably want to cluster similar flops together with it. So using something closes to the original betting frequencies, such as the transformed Bet 3 and Bet 5.9 variables, makes more sense as clustering variables. If we want to identify flops by how unusual they are because we want to study them more closely than others, we can simply sort the table by Mahalanobis distance.
