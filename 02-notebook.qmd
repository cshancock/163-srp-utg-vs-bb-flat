...
add_compositional_distance <- function(df, action_cols) {
        ...
        map_dfc(~{
            ...
            df |>
                transmute(
                    ...
                    !!col_name := sqrt(!!sym(.) + 1e-6) / sqrt(!!sym(ref_col) + 1e-6)
                )
        })
    ...
    T2 <- mahalanobis(X, center = colMeans(X), cov = cov_matrix)
    
    # Add to original dataframe
    df |> 
        bind_cols(transformed) |>
        mutate(mahalanobis = T2)
}
...
**Interpretation:**
- **Low values (close to 0)**: The observation is similar to the typical observation in the distribution, indicating good balance or similarity between groups
- **High values**: The observation is far from the typical observation, indicating poor balance or dissimilarity between groups

**Why transform with square root or log1p():**
- Mahalanobis distance is always non-negative and often has a highly right-skewed distribution
- **Square root transformation**: Reduces skewness and makes the distribution more symmetric, which can improve visualization and interpretation
- **log1p() transformation**: `log(1 + x)` handles zero values gracefully while compressing large distances, making extreme outliers less influential and creating a more interpretable scale for visualization
- Both transformations help create more evenly distributed visual scales when plotting balance diagnostics
